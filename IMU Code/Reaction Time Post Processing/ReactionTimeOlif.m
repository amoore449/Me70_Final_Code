...
if t == 1
    if d == 1
        Data = readtable('Trial01A_Olif_AccelerationX.xlsx');
        trial_time = table2array(trial_times(1,[2:5]));
    end
    if d == 2
        Data = readtable('Trial01B_Olif_AccelerationX.xlsx');
        trial_time = table2array(trial_times(2,[2:5]));
    end
    if d == 3
        Data = readtable('Trial01C_Olif_AccelerationX.xlsx');
        trial_time = table2array(trial_times(3,[2:5]));
    end 
end
if t == 2
    if d == 1
         Data = readtable('Trial02A_Olif_AccelerationX.xlsx');
         trial_time = table2array(trial_times(4,[2:5]));
    end
    if d == 2
        Data = readtable('Trial02B_Olif_AccelerationX.xlsx');
        trial_time = table2array(trial_times(5,[2:5]));
    end
    if d == 3
        Data = readtable('Trial02C_Olif_AccelerationX.xlsx');
        trial_time = table2array(trial_times(6,[2:5]));
    end 
end
if t == 3
    if d == 1
         Data = readtable('Trial03A_Olif_AccelerationX.xlsx');
         trial_time = table2array(trial_times(7,[2:5]));
    end
    if d == 2
        Data = readtable('Trial03B_Olif_AccelerationX.xlsx');
        trial_time = table2array(trial_times(8,[2:5]));
    end
    if d == 3
        Data = readtable('Trial03C_Olif_AccelerationX.xlsx');
        trial_time = table2array(trial_times(9,[2:5]));
    end 
end
...
if d == 1
    sgtitle("No Distraction(Olif)")
end
if d == 2
    sgtitle("Texting Distraction(Olif)")
end
if d == 3
    sgtitle("Gaming Distraction(Olif)")
end
...
%Decided to neglect the last trial  for this distraction mode because it 
%is unclear from the acceleration data when Olif stopped
timeDifferenceMeansC = mean(timeDifferenceArray(7:8));
...
figure(4)
hold on
title("Reaction Time for Different Distraction Conditions (Olif)")
